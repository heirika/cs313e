...
class Graph:
    ...
    def has_cycle(self):
        ...
        #helper function for checking cycle: DFS implementation (stack)
        def dfs_cycle(vertex_index, path):
            #current vertex = visited
            self.vertices[vertex_index].visited = True
            path.append(vertex_index)

            #check adjacent vertices
            for neighbor in self.get_adjacent_vertices(vertex_index):
            ...
            #all adj verteices explored: remove current vertex, explore others
            path.pop()
            return False

        #check each vertices with DFS recursively
    ...
    def get_registration_plan(self):
        ...
        #while list not fully processed
        while temp_vertices:
            sem_courses = []
            added_courses = []

            # check if current has prereq: break if prereq found
            for current, vertex in enumerate(temp_vertices):
                has_prereq = False
                for row in range(len(temp_matrix)):
                    if temp_matrix[row][current] == 1:
                        has_prereq = True
                        break
                #does not have prereq: add class
                if not has_prereq:
                    sem_courses.append(vertex.label)
                    added_courses.append(vertex)

                #max of 4 classes per sem
                if len(sem_courses) == 4:
                    break

            # if class added: remove from matrix
            for course in added_courses:
                delete_vertex_from_copy(course.label, temp_matrix, temp_vertices)

           #if sem courses ready: add to courses
            if sem_courses:
