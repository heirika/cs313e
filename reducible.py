...
def step_size(s):
    ...
    step = STEP_SIZE_CONSTANT - hash_word(s,STEP_SIZE_CONSTANT)
    return step
...
def insert_word(s, hash_table):
    ...
    #no collision: insert s into hash table
    index = hash_word(s, table_size)
    if hash_table[index] == "":
        hash_table[index] = s
        return
    #duplicate exists: no need to insert
    elif hash_table[index] == s:
        return
    #if collision: double hash and try to insert
    step = step_size(s)

    #while we have not found an empty slot: keep adding step
...
def is_reducible(s, hash_table, hash_memo):
    ...
    """
    #base case 1: reduced to a, i, o
    if s in ["a", "i", "o"]:
        return find_word(s, hash_table)
    #base case 2: if found in hash memo: return True
    ...
    #recursive: reduce at index i, if reducible return True
...
def main():
    ...
    longest_reduce_words = get_longest_words(reducible_words)

    # print the reducible words in alphabetical order
    # one word per line
    sorted_reduce_words = sorted(longest_reduce_words)
    for word in sorted_reduce_words:
        print(word)
