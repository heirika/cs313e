...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        for i, letter in enumerate(guessed_word):
            ...
            #same as correct color, change color
            if current_color == CORRECT_COLOR:
                self.colors[letter] = CORRECT_COLOR
            #if current color is not correct, change to wrong spot
            elif current_color == WRONG_SPOT_COLOR:
                if self.colors[letter] != CORRECT_COLOR:
                    self.colors[letter] = WRONG_SPOT_COLOR
            #letter not in word
            elif current_color == NOT_IN_WORD_COLOR:
                #change color if current is uncolored
    ...
    def __str__(self):
        ...
        for row_index, row in enumerate(keyb_rows):
            output += " " * row_spaces[row_index]

            for letter in row:
                color = self.colors.get(letter, NO_COLOR)
                output += color_word(color, letter) + " "
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    list_secretword = list(secret_word)
    ...
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
            ...
            list_secretword[i] = None

  #correct letter, wrong place
    for i in range(NUM_LETTERS):
        if feedback[i] == NOT_IN_WORD_COLOR:
            if guessed_word[i] in list_secretword:
                feedback[i] = WRONG_SPOT_COLOR
                list_secretword.remove(guessed_word[i])
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    word_fams = defaultdict(list)

    for word1 in remaining_secret_words:
        #get feedback colors
        feedback_colors = get_feedback_colors(word1, guessed_word)

        #create tuple to use as key for words_fam dictionary
        feedback_tuple = tuple(feedback_colors)

        #initalize dictionary, using tuple as keys
        word_fams[feedback_tuple].append(word1)
    ...
    #update based on: fam size, difficulty, lexicon order
        if (
            fam_size > max_size or
            #same size, more difficult
            (fam_size == max_size and difficulty > max_diff) or
            #same size, diff, less hard color
            (fam_size == max_size and difficulty == max_diff and feedback_tuple < hardest_feedback)
            ):

            hardest_fam = words2
            hardest_feedback = feedback_tuple
            max_size = fam_size
            max_diff = difficulty

    #return feedback colors for hardest family & remaining secret words
    return hardest_feedback, hardest_fam
