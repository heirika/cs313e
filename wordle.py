...
#set secret word to none to prevent raising incorrect valueerrors
    secret_word = None

    if len(sys.argv) == 2:
        arg = sys.argv[1]
        # if digit, use to set seed and choose word
        if arg.isdigit():
            random.seed(int(arg))
            secret_word = random.choice(secret_words)
        #check length, lower, and is word
        elif len(arg) == 5 and arg.islower() and arg.isalpha():
            if arg in valid_words:
                secret_word = arg
        else:
            raise ValueError("Bad input detected. Please try again.")
    else:
        raise ValueError("Bad input detected. Please try again.")
    # #default word to hello
...
def is_valid_guess(guess, valid_guesses):
    ...
    if isinstance(guess, str) and len(guess) == 5 and guess.islower() and guess in valid_guesses:
        return True
    else:
        return False
